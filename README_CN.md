...
## Overview
The RealHand ROS SDK is developed by Lingxin Qiaoshou (Beijing) Technology Co., Ltd. It provides drivers and example source code for RealHand dexterous hands such as O6, L6, L7, O7, L10, and L21. It can be used with real hardware and simulators.  
The RealHand ROS2 SDK currently supports Ubuntu 22.04, ROS Humble, and Python 3.10 or later.
...
## Usage for Ubuntu
  **Before using, edit the [setting.yaml](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/real_hand_ros2_sdk/real_hand_ros2_sdk/RealHand/config/setting.yaml) configuration file according to your actual needs.**  
...
  **Before using, edit the single-hand [real_hand.launch.py](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/real_hand_ros2_sdk/launch/real_hand.launch.py) or dual-hand [real_hand_double.launch.py](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/real_hand_ros2_sdk/launch/real_hand_double.launch.py) files according to your actual hand parameters.**

- Start SDK (single hand)   Plug the RealHand USB-to-CAN device into the Ubuntu machine. Supported models: O6/L6/L7/L10/L20/L21/L25  
- Start SDK (dual hands)   First plug the left-hand RealHand USB-to-CAN device into the Ubuntu machine (usually recognized as `can0`). Then plug the right-hand device (usually recognized as `can1`). Supported models: O6/L6/L7/L10/L20/L21/L25
    ...
    $ sudo chmod a+x src/real_hand_ros2_sdk/real_hand_ros2_sdk/real_hand_ros2_sdk/real_hand.py
    $ # Single hand
    $ ros2 launch real_hand_ros2_sdk real_hand.launch.py
    $ # Dual hands
    $ ros2 launch real_hand_ros2_sdk real_hand_double.launch.py
...
  **Before using, edit the [real_hand.launch.py](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/real_hand_ros2_sdk/launch/real_hand.launch.py) file according to your actual hand parameters.**

- Start SDK   Plug the RealHand USB-to-CAN device into the Windows machine. Supported models: L7/L10/L20/L21/L25  
- Note: You must install the USB-to-CAN driver before use.
    ...
    $ ros2 launch real_hand_ros2_sdk real_hand.launch.py # Edit the CAN port name in the launch file first
    $ [real_hand_sdk-1] 2025-06-24 17:21:14  Current SDK version: 2.1.4
    $ [real_hand_sdk-1] 2025-06-24 17:21:14  left L10 set speed to [200, 250, 250, 250, 250, 250, 250, 250, 250, 250]
    $ [real_hand_sdk-1] 2025-06-24 17:21:14  left L10 set maximum torque to [200, 200, 200, 200, 200]
...
Edit `config/setting.yaml` according to the comments in the file. Set `MODBUS: "/dev/ttyUSB0"`, and set the `"modbus"` parameter in the [real_hand.launch.py](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/real_hand_ros2_sdk/launch/real_hand.launch.py) configuration file to `"/dev/ttyUSB0"`. The USB-RS485 adapter usually appears as `/dev/ttyUSB*` or `/dev/ttyACM*` on Ubuntu.  
...
- > ### release_2.1.7
  ...
  - 2. Moved Mujoco and PyBullet simulation to a separate repository to reduce the SDK size
...
- > ### release_1.0.2
  ...
  - 3. Added RealHand waveform display for hands with pressure sensors
...
The GUI control can use sliders to control each joint of the RealHand L10 and L20 independently. You can also record the current values of all sliders with buttons, saving the current joint state of the RealHand, and replay actions via functional buttons.    

Using `gui_control` to control the RealHand:  
The `gui_control` interface requires `real_hand_sdk_ros` to be running and controls the RealHand via topics.  
After starting the ROS2 SDK:

  **Before using, edit the [gui_control.launch.py](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/gui_control/launch/gui_control.launch.py) file according to your actual hand parameters.**
...
After launching, a UI window will pop up. You can control the corresponding RealHand joints via the sliders.

- Add or modify action examples. You can add or modify actions in the [constants.py](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/gui_control/gui_control/config/constants.py) file.
...
The matrix pressure heatmap displays fingertip matrix pressure sensor data for each joint of the RealHand and visualizes it as a heatmap. Make sure your hand is equipped with matrix pressure sensors before using.  
After starting the ROS2 SDK:  
  **Before using, edit the [matrix_touch_gui.launch.py](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/matrix_touch_gui/launch/matrix_touch_gui.launch.py) file according to your actual hand parameters.**
...
## L7
- [7001-action-group-show-ti (Finger movements)](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/examples/L7/gesture/action-group-show-ti.py)

## L10
- [10001-action-group-show-normal (Finger movements)](https://github.com/XXXXXXDU/realhand-ros2-sdk/blob/main/examples/L10/gesture/action-group-show-normal.py)
