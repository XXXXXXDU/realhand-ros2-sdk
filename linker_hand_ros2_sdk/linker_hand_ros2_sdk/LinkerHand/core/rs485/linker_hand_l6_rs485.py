...
class LinkerHandL6RS485:
    ...
    def __init__(self, hand_id=0x27, modbus_port="/dev/ttyUSB0", baudrate=115200):
        ...
        # pymodbus 3.5.1 requires an explicit connect
    ...
    def read_versions(self) -> Dict[str, int]:
        """Read version information (input registers 148–155)"""
    ...
    def write_angles(self, vals: List[int]):
        ...
        if not self.is_valid_6xuint8(vals):
            raise ValueError("Require six integers in the range 0–255")
    ...
    def is_valid_6xuint8(self, lst) -> bool:
        """Validate a list of six integers in the range 0–255"""
        if len(lst) != 6:
            return False
    ...
    def set_current(self, current=None):
        """Set current (not supported on L6)"""
        print("Setting current is not supported on L6", flush=True)
    ...
    def get_current(self):
        """Get current (not supported on L6)"""
        print("Getting current is not supported on L6", flush=True)
        return []
        ...
        try:
            ...
            for i, name in enumerate(self.JOINT_NAMES):
                print(f"  {name:15s}: angle={angles[i]:3d}, torque={torques[i]:3d}, "
                      f"speed={speeds[i]:3d}, temp={temps[i]:2d}°C, error={errors[i]:2d}")
